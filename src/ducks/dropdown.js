...
export const getCategoryByRating = (payload) => ({ type: GET_CATEGORY_BY_RATING, payload });
...
export const getCategoryByRatingAction = () => (dispatch) => {
    return handleFetch(`category?nested=false&byRating=true`, 'GET')
        .then((res) => {
            if (!res.success) {
                throw res.payload;
            } else {
                dispatch(getCategoryByRating(res.payload.slice(0, 6)));
            }
        })
        .catch((e) => handleErrorType(e));
};
